	...
	<div id="container">
		<div class="common_zone">
			...
			<div class="super_tab">
				...
				<label for="t2" class="new_point">1:1 문의</label>
				...
				<section id="con2">
					<div class="comInterview_contents">
						<div id="tab2" class="demoTab">
							<ul class="tab_lst">
								<li class="on"><a href="#">1:1 문의내역</a></li>
								<li><a href="#">1:1 문의하기</a></li>
							</ul>
							<div class="group">
							
								<div class="on tab_2 inq_lst">			
									<section>
										<table>
										</table>
									</section>
		...
		$('table').eq(0).DataTable({
		     ...
		     columns: [
				{"data": function (data, type, dataToSet) { 
					   ...
					   list += '<div><span>'+data.faTitle+'</span><span>'+data.faSubject+'</span></div>';
		...
		$('table').eq(1).DataTable({
		     ...
		     columns: [
				{"data": function (data, type, dataToSet) { 
					   var list = '<section>';
					   list += '<section class="div_wrapper">';
					   list += '<div><span>[이용방법]</span><span>'+data.faSubject+'</span></div>';
					   list += '<div><span>'+data.faDatetime+'</span><button class="toggle_a"></button></div>';
					   list += '</section>';
					   list += '<div class="slideToggle_p none">';
					   list += '<h4>'+data.faSubject+'</h4>';
		...
		$('table').eq(2).DataTable({
		     ...
		     ajax: {
		        ...
		        dataSrc: function (data) {
		        	...
		        	$(".totalRow").text(data.length);
	                if (data.length == 0) {
	                	$(".dataTables_paginate").eq(2).addClass("none");
	                	data = "";
		     ...
		     columns: [
				{"data": function (data, type, dataToSet) { 
					   var list = '<section>';
					   list += '<section class="div_wrapper">';
					   list += '<div><span>[회원정보]</span><span>'+data.faSubject+'</span></div>';
					   list += '<div><span>'+data.faDatetime+'</span><button class="toggle_a"></button></div>';
					   list += '</section>';
					   list += '<div class="slideToggle_p none">';
					   list += '<h4>'+data.faSubject+'</h4>';
		...
		$('table').eq(3).DataTable({
		     ...
		     ajax: {
		        ...
		        dataSrc: function (data) {
		        	...
		        	$(".totalRow").text(data.length);
	                if (data.length == 0) {
	                	$(".dataTables_paginate").eq(3).addClass("none");
	                	data = "";
		     ...
		     columns: [
				{"data": function (data, type, dataToSet) { 
					   var list = '<section>';
					   list += '<section class="div_wrapper">';
					   list += '<div><span>[결제]</span><span>'+data.faSubject+'</span></div>';
					   list += '<div><span>'+data.faDatetime+'</span><button class="toggle_a"></button></div>';
					   list += '</section>';
					   list += '<div class="slideToggle_p none">';
					   list += '<h4>'+data.faSubject+'</h4>';
		...
		$('table').eq(4).DataTable({
		     ...
		     ajax: {
		        ...
		        dataSrc: function (data) {
		        	...
		        	$(".totalRow").text(data.length);
	                if (data.length == 0) {
	                	$(".dataTables_paginate").eq(4).addClass("none");
	                	data = "";
		     ...
		     columns: [
				{"data": function (data, type, dataToSet) { 
					   var list = '<section>';
					   list += '<section class="div_wrapper">';
					   list += '<div><span>[기타]</span><span>'+data.faSubject+'</span></div>';
					   list += '<div><span>'+data.faDatetime+'</span><button class="toggle_a"></button></div>';
					   list += '</section>';
					   list += '<div class="slideToggle_p none">';
					   list += '<h4>'+data.faSubject+'</h4>';
			...
			if(userId == ""){
				...
				$.ajax({
			        ...
			        success:function(data){  
			        	alert("등록이 완료되었습니다.");
			        	location.href = "${pageContext.request.contextPath}/board/help2";	 
			        },
	    ...
	    $('table').eq(5).DataTable({
		     ...
		     ajax: {
		        ...
		        dataSrc: function (data) {
	                ...
	                if (data.length == 0) {
	                	...
	                	$(".dataTables_paginate").eq(5).addClass("none");
	                	data = "";
		     ...
		     columns: [
				{"data": function (data, type, dataToSet) { 
						var list = '<section>';
						list += '<section class="div_wrapper">';
						list += '<div><span>['+data.fsName+']</span><span><p class="new_post">'+data.inSubject+'</p></span></div>';
