...
import AvMenu from '/resources/lib/AvMenu.js';
import Settings from '/resources/lib/Settings.js';
import WowzaWebRTCPublish from '/resources/lib/WowzaWebRTCPublish.js';
window.WowzaWebRTCPublish = WowzaWebRTCPublish;
let browserDetails = window.adapter.browserDetails;

$(document).ready(() => {
	
	//$('#sdpURL').val('wss://611a3faad4c7d.streamlock.net/webrtc-session.json');
	document.getElementById("sdpURL").value = 'wss://611a3faad4c7d.streamlock.net/webrtc-session.json';
  ...
  let state = {
    ...
    selectedMic: '',
    settings: {
      sdpURL: "wss://611a3faad4c7d.streamlock.net/webrtc-session.json",
      applicationName: "",
      streamName: "",
      audioBitrate: "64",
      audioCodec: "opus",
  ...
  const getState = () => {
	console.log('getState state value ->>' , state);
    return state;
  }

  // throw errors with these messages
  const okToStart = () => {
	var wss = 'wss://611a3faad4c7d.streamlock.net/webrtc-session.json';
	state.settings.sdpURL = wss;
  ...
  const updatePublisher = () => {
    let publishSettings = Settings.mapFromForm
		( Settings.serializeArrayFormValues($( "#publish-settings-form" )) );
			console.log('publishSettings ->>>' , publishSettings);	
